...
public class WysiwygHTMLEditorKit extends HTMLEditorKit {
	...
	private class ResizeHandler extends MouseInputAdapter {
		...
		private void replace(Element elem, String html) {
			...
			try {
				...
				System.err.println("WysiwygHTMLEditorKit.replace("+elem.getName()+","+html+")");
			}
	...
	/**
	 * A view for {@link Element}s that are uneditable. <p> The default view for
	 * such {@link Element}s are big ugly blocky JTextFields, which doesn't
	 * really work well in practice and tends to confuse users. This view
	 * replaces the default and simply draws the elements as blocks that
	 * indicates the presence of an uneditable {@link Element}. <p>
	 *
	 * @author Bob Tantlinger
	 *
	 */
