...
public class EditItem extends javax.swing.JPanel {
    ...
    public boolean isModified() {
        if (!item.getName().equals(txName.getText())) return(true);
        if (!item.getDescription().equals(description.getText())) return(true);
        if (!item.getNotes().equals(notes.getText())) return(true);
        if (!item.getCategory().equals(cbCategory.getSelectedItem().toString())) return(true);
	return(false);
    }
    ...
    private String ctrlData() {
        ...
        if (!"".equals(description.getText())) return(mandatoryField("msg.common.description"));
        return ("");
    }
}
