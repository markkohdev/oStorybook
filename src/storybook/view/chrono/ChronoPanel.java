...
import storybook.view.linkspanel.LocationLinksPanel;
import storybook.view.linkspanel.PersonLinksPanel;
import storybook.view.linkspanel.StrandLinksPanel;
import storybook.view.options.ChronoOptionsDialog;
...
public class ChronoPanel extends AbstractScrollPanel implements Printable,
		MouseWheelListener {
	...
	public void modelPropertyChange(PropertyChangeEvent evt) {
		...
		if (DocumentController.StrandProps.UPDATE.check(propName)) {
			for (JLabel lb : strandLabels) {
				...
				Strand strand = (Strand) EntityUtil.get(mainFrame,
						Strand.class, id);
	...
	public void refresh() {
		...
		// List<Date> dates = sceneDao.findDistinctDates();
		...
		if (layoutDirection == true) {
			...
			for (Strand strand : strands) {
				...
				for (Date date : dates) {
					if (showDateDiff && lastDate != null) {
						DateDiffLabel lbDiff = new DateDiffLabel(lastDate,
								date, true);
					...
					RowPanel rowPanel = new RowPanel(mainFrame, strand,
							date);
	...
	private static void dispatchToChronoScenePanels(Container cont,
	...
	private static void dispatchToPersonLinksPanels(Container cont,
			PropertyChangeEvent evt) {
		List<Component> ret = new ArrayList<Component>();
	...
	private static void dispatchToLocationLinksPanels(Container cont,
			PropertyChangeEvent evt) {
		List<Component> ret = new ArrayList<Component>();
	...
	private static void dispatchToStrandLinksPanels(Container cont,
			PropertyChangeEvent evt) {
		List<Component> ret = new ArrayList<Component>();
	...
	private static void dispatchToSpacePanels(Container cont,
			PropertyChangeEvent evt) {
		List<Component> ret = new ArrayList<Component>();
