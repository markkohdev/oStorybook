...
public class ChronoScenePanel extends AbstractScenePanel implements FocusListener {
	...
	public void refresh() {
		...
		if (scene.hasChapter()) {
			if (scene.getChapter().getPart().getId() != mainFrame
		...
		// person links
		PersonLinksPanel personLinksPanel = new PersonLinksPanel(mainFrame,
				scene);

		// location links
		LocationLinksPanel locationLinksPanel = new LocationLinksPanel(
				mainFrame, scene);
		...
		if (scene.getInformative()) {
			...
			lbInformational.setToolTipText(I18N
					.getMsg("msg.common.informative"));
		}
