...
public abstract class AbstractOptionsDialog extends AbstractDialog implements
	...
	private void internalInitUi() {
		...
		panel.setLayout(new MigLayout("fill,wrap 2,ins 20", "[]20[grow]"));
		panel.setBorder(SwingUtil.getBorderDefault());

		if (zoom) {
			// zoom
			JLabel lbZoom = new JLabel(I18N.getMsgColon("msg.common.zoom"));
			JSlider zoomSlider = new JSlider(JSlider.HORIZONTAL, zoomMinValue,
					zoomMaxValue, zoomValue);
		...
		panel.add(getCloseButton(), "span 2,pushy 200,al right bottom");
