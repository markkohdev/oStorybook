...
public class EntityUtil {
	...
	public static Boolean isPersonAlive(Person person, Date now) {
		...
		return now.after(person.getDayofdeath()) ? false : true;
	}
	...
	public static void convertPlainTextToHtml(MainFrame mainFrame) {
		...
		for (Chapter chapter : chapters) {
			if (useHtmlDescr) {
				chapter.setDescription(HtmlUtil.textToHTML(chapter
						.getDescription()));
		...
		for (Person person : persons) {
			if (useHtmlDescr) {
				person.setDescription(HtmlUtil.textToHTML(person
						.getDescription()));
		...
		for (Location location : locations) {
			if (useHtmlDescr) {
				location.setDescription(HtmlUtil.textToHTML(location
						.getDescription()));
	...
	public static void convertHtmlToPlainText(MainFrame mainFrame) {
		...
		for (Item item : items) {
			if (usePlainTextDescr) {
				item.setDescription(HtmlUtil.htmlToText(item.getDescription(),
						true));
...
//		AbstractEntity newEntity = cloneEntity(mainFrame, entity);
		...
		if (entity instanceof Scene) {
			Scene scene = (Scene) entity;
			List<Person> copyPersons = new ArrayList<Person>();
			for (Person person : persons) {
				copyPersons.add(person);
			}
			scene.setPersons(copyPersons);
			List<Location> copyLocations = new ArrayList<Location>();
			for (Location location : locations) {
				copyLocations.add(location);
			}
			scene.setLocations(copyLocations);
			List<Strand> copyStrands = new ArrayList<Strand>();
	...
	public static boolean hasHierarchyChanged(AbstractEntity oldEntity,
			AbstractEntity updEntity) {
		...
		if (oldEntity instanceof Idea) {
			...
			if (old.getStatus() != upd.getStatus()) {
				return true;
			}
			return false;
		}
		if (oldEntity instanceof Scene) {
			...
			if (old.getChapter() != null && upd.getChapter() != null
					&& (old.getChapter().getId() != upd.getChapter().getId())) {
				return true;
			}
			return false;
		}
		if (oldEntity instanceof Person) {
			...
			if (old.getCategory().getId() != upd.getCategory().getId()) {
				return true;
			}
			if (old.getGender().getId() != upd.getGender().getId()) {
				return true;
			}
			return false;
		}
		if (oldEntity instanceof Location) {
			...
			if (!oldCountry.equals(updCountry)) {
				return true;
			}
			return false;
		}
		if (oldEntity instanceof AbstractTag) {
			...
			if (!old.getCategory().equals(upd.getCategory())) {
				return true;
			}
			return false;
		}
	...
	public static boolean isAvailableInMemoria(AbstractEntity entity) {
		if (entity instanceof Person || entity instanceof Location
	...
	public static void setEntityAttributes(MainFrame mainFrame,
		...
		if (entity instanceof Person) {
			try {
				...
				// delete attributes
				Session session = model.beginTransaction();
				person.setAttributes(new ArrayList<Attribute>());
	...
	private static void toolTipAppendLocation(StringBuffer buf,
			Location location) {
	...
	private static String getInfo(MainFrame mainFrame, AbstractEntity entity,
			boolean truncate) {
		StringBuffer buf = new StringBuffer();
		...
		if (entity instanceof Category) {
			...
			if (!persons.isEmpty()) {
				...
				buf.append(HtmlUtil.getWarning(I18N
						.getMsg("msg.category.delete.warning")));
				buf.append("</p><br>");
			...
			if (!persons.isEmpty()) {
				buf.append("<p style='padding-top:10px'>");
				buf.append(HtmlUtil.getWarning(I18N
						.getMsg("msg.gender.delete.warning")));
				buf.append("</p><br>");
				buf.append(I18N.getMsgColon("msg.common.persons"));
				buf.append("<br><ul>");
				for (Person person : persons) {
			...
			if (!scenes.isEmpty()) {
				buf.append("<p style='padding-top:10px'>");
				buf.append(HtmlUtil.getWarning(I18N
						.getMsg("msg.relativedate.delete.warning")));
				buf.append("</p><br>");
				buf.append(I18N.getMsgColon("msg.common.scenes"));
				buf.append("<br><ul>");
			...
			if (!chapters.isEmpty()) {
				buf.append("<p style='padding-top:10px'>");
				buf.append(HtmlUtil.getWarning(I18N
						.getMsg("msg.dlg.mng.parts.delete.warning")));
				buf.append("</p><br>");
			...
			if (!scenes.isEmpty()) {
				buf.append("<p style='padding-top:10px'>");
				buf.append(HtmlUtil.getWarning(I18N
						.getMsg("msg.dlg.mng.strands.delete.warning")));
				buf.append("</p><br>");
				buf.append(I18N.getMsgColon("msg.common.scenes"));
				buf.append("<br><ul>");
				for (Scene scene : scenes) {
			...
			if (!links.isEmpty()) {
				buf.append("<p style='padding-top:10px'>");
				buf.append(HtmlUtil.getWarning(I18N
						.getMsg("msg.tags.links.warning.delete.all")));
				buf.append("</p><br>");
			...
			if (!links.isEmpty()) {
				buf.append("<p style='padding-top:10px'>");
				buf.append(HtmlUtil.getWarning(I18N
						.getMsg("msg.item.links.warning.delete.all")));
				buf.append("</p><br>");
	...
	private static void addInfo(MainFrame mainFrame, AbstractEntity entity,
			StringBuffer buf, boolean truncate) {
			...
			try {
				...
				if (ret != null) {
					...
					if (ret instanceof Boolean) {
						str = ((Boolean) ret ? I18N.getMsg("msg.common.yes")
								: I18N.getMsg("ms" +
										"g.common.no"));
					...
					if (truncate) {
						str = TextUtil.truncateString(HtmlUtil.htmlToText(str),
								200);
					}
					buf.append(str);
				}
	...
	public static void fillAutoCombo(MainFrame mainFrame,
		...
		try {
			...
			List<String> items = (List<String>) m.invoke(dao);
			model.commit();
