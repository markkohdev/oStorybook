...
//				break;
	...
	return getHumanReadableByteCount(getMemoryUsed()) + " / "
	...
	if (DocumentUtil.isUseHtmlScenes(mainFrame)) {
	    ...
	    ep.setEditorKitForContentType("text/html",
    ...
    public static void replaceComponent(Container cont, int index,
	    Component comp) {
	cont.remove(index);
	// cont.validate();
	cont.add(comp, index);
    ...
    public static void replaceComponent(Container cont, Component comp1,
	    Component comp2) {
	replaceComponent(cont, comp1, comp2, "");
    }

    public static void replaceComponent(Container cont, Component comp1,
	    Component comp2, String constraints) {
	int index = -1;
	int i = 0;
	...
	comp.setPreferredSize(new Dimension(Integer.MAX_VALUE,
		Integer.MAX_VALUE));
    }
	...
	HashMap<Object, Action> actions = SwingUtil
     ...
     *
     * eg. setUIFont(new javax.swing.plaf.FontUIResource ("Serif",Font.ITALIC,12));
     */
	...
	if (rootComponent instanceof Container) {
		...
		if (comp instanceof Container) {
		    findComponentsNameStartsWith((Container) comp, startsWith,
			    res);
		}
	    }
	}
